...
def test_index_redirects_to_signin(client):
    response = client.get("/")
    assert response.status_code == 302
    assert "/signin" in response.headers["Location"]

@patch("app.get_db")  
def test_admin_login_success(mock_get_db, client):
    mock_db = MagicMock()
...
@patch("app.get_db")
def test_process_signin_success(mock_get_db, mock_post, client):
    ...
    response = client.post("/process_signin", data={"image": "dummy_base64"})
    assert response.status_code == 200
    assert response.json["success"] is True
    assert "/signin/success/" in response.json["redirect"]

@patch("app.requests.post")
def test_process_signin_failure(mock_post, client):
    mock_post.return_value.status_code = 200
    mock_post.return_value.json.return_value = {
        "success": False,
        "verified": False,
